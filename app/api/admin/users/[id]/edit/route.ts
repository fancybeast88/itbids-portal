...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (user.vendorProfile) {
    ...
    if (creditsToAdd && creditsToAdd !== 0) {
      await prisma.$transaction([
        prisma.vendorProfile.update({ where: { userId: id }, data: { credits: { increment: creditsToAdd } } }),
        prisma.creditTransaction.create({ data: { vendorId: user.vendorProfile.id, type: 'manual', credits: creditsToAdd, status: 'confirmed' } }),
      ])
    }
  }
  ...
  if (user.businessProfile) {
    ...
    if (creditsToAdd && creditsToAdd !== 0) {
      await prisma.$transaction([
        prisma.businessProfile.update({ where: { userId: id }, data: { credits: { increment: creditsToAdd } } }),
        prisma.businessCreditTransaction.create({ data: { bizId: user.businessProfile.id, type: 'manual', credits: creditsToAdd, status: 'confirmed' } }),
      ])
    }
  }
