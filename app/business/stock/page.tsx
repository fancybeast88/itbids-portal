...
export default function BusinessStockPage() {
  ...
  const filtered = items.filter((i:any) => {
    ...
    if (search) {
      ...
      return i.model?.toLowerCase().includes(s) || i.brand?.toLowerCase().includes(s) || i.vendor?.companyName?.toLowerCase().includes(s)
    }
    return true
  })
    ...
    <PortalLayout bizCredits={bizCredits}>
      <div className="p-6">
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-16 bg-white rounded-xl border border-gray-100 text-gray-400 text-sm">No stock items found.</div>
        )}
        <div className="grid grid-cols-2 gap-3">
          {filtered.map((item:any) => (
            <div key={item.id} className={"bg-white border-2 rounded-xl p-4 " + (typeColor[item.type] || 'border-gray-100')}>
              ...
              <div className="text-xs text-gray-400">{item.category} · {item.city}</div>
              {item.description && <div className="text-xs text-gray-500 mt-1.5 leading-relaxed">{item.description}</div>}
              <div className="flex gap-4 mt-2 text-xs flex-wrap">
                <span><strong className="text-gray-600">Qty:</strong> {item.quantity}</span>
                {item.unitPricePkr && <span><strong className="text-gray-600">PKR</strong> {Number(item.unitPricePkr).toLocaleString('en-PK')}</span>}
                {item.type === 'upcoming' && item.expectedDate && (
                  <span><strong className="text-gray-600">Expected:</strong> {new Date(item.expectedDate).toLocaleDateString('en-PK', { day:'numeric', month:'short', year:'numeric' })}</span>
                )}
              </div>
              <div className="mt-3 pt-3 border-t border-gray-50 flex items-center justify-between">
                ...
                {item.vendor?.phone && (
                  <a href={"tel:" + item.vendor.phone} className="text-xs px-3 py-1 bg-blue-600 text-white rounded-lg">Contact</a>
                )}
              </div>
            </div>
          ))}
