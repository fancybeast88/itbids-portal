...
function downloadPDF(q: any, rfqTitle: string, bizName: string) {
  ...
  const lineRows = lines.map((item: any) => `
    <tr>
      <td style="padding:8px 10px;border-bottom:1px solid #f0f0f0">${item.desc || item.description || '—'}</td>
      <td style="padding:8px 10px;border-bottom:1px solid #f0f0f0;text-align:center">${item.sku || '—'}</td>
      <td style="padding:8px 10px;border-bottom:1px solid #f0f0f0;text-align:center">${item.qty || item.quantity || 1}</td>
      <td style="padding:8px 10px;border-bottom:1px solid #f0f0f0;text-align:right">PKR ${fmt(item.unitPrice || item.unit_price || 0)}</td>
      <td style="padding:8px 10px;border-bottom:1px solid #f0f0f0;text-align:right;font-weight:600">PKR ${fmt((item.qty || item.quantity || 1) * (item.unitPrice || item.unit_price || 0))}</td>
    </tr>`).join('')

  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8">
  <title>Quote — ${rfqTitle}</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:Arial,sans-serif;color:#1a1a2e;font-size:13px;padding:40px}
    .header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:32px;padding-bottom:20px;border-bottom:2px solid #1A56DB}
...
function downloadPDF(q: any, rfqTitle: string, bizName: string) {
  ...
  <div class="header">
    <div><div class="brand">Lead Vault</div><div class="brand-sub">Pakistan IT Procurement Marketplace</div></div>
    <div><div class="doc-title">PROFORMA INVOICE / QUOTE</div><div class="doc-date">Date: ${date}</div></div>
  </div>
  <div class="section"><div class="section-title">RFQ</div>
    <div class="info-box"><div class="info-label">RFQ Title</div><div class="info-value">${rfqTitle}</div></div>
  </div>
  <div class="section grid2">
    <div><div class="section-title">Vendor (Supplier)</div>
      <div class="info-box">
        <div class="info-label">Company</div><div class="info-value">${q.vendor?.companyName || '—'}</div>
        <div style="margin-top:8px"><div class="info-label">Contact</div><div class="info-value">${q.vendor?.contactPerson || '—'}</div></div>
        <div style="margin-top:8px"><div class="info-label">Phone</div><div class="info-value">${q.vendor?.phone || '—'}</div></div>
        <div style="margin-top:8px"><div class="info-label">City</div><div class="info-value">${q.vendor?.city || '—'}</div></div>
        <div style="margin-top:8px"><div class="info-label">Partner level</div><div class="info-value">${q.vendor?.partnerLevel || '—'}</div></div>
      </div>
    </div>
    <div><div class="section-title">Business (Buyer)</div>
      <div class="info-box"><div class="info-label">Company</div><div class="info-value">${bizName}</div></div>
    </div>
  </div>
  <div class="section"><div class="section-title">Line Items</div>
    <table>
      <thead><tr>
        <th>Description</th><th>SKU/Model</th>
      ...
      <tbody>
        ...
        <tr class="total-row">
          <td colspan="4" style="text-align:right;padding:12px 10px;font-size:13px">TOTAL AMOUNT</td>
          <td style="text-align:right;padding:12px 10px">PKR ${fmt(Number(q.totalAmount))}</td>
        </tr>
      </tbody>
    </table>
  </div>
  <div class="terms">
    <div class="term-box"><div class="info-label">Delivery</div><div class="info-value">${q.deliveryDays || '—'}</div></div>
    <div class="term-box"><div class="info-label">Warranty</div><div class="info-value">${q.warranty || '—'}</div></div>
    <div class="term-box"><div class="info-label">Payment terms</div><div class="info-value">${q.paymentTerms || '—'}</div></div>
    <div class="term-box"><div class="info-label">Quote validity</div><div class="info-value">${q.validityDays ? q.validityDays + ' days' : '—'}</div></div>
    <div class="term-box" style="grid-column:span 2"><div class="info-label">Notes</div><div class="info-value">${q.notes || '—'}</div></div>
  </div>
  <div class="footer">Generated via Lead Vault · leadvault.pk · Pakistan IT Procurement Marketplace</div>
</body></html>`
            ...
            <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50/50"
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-400">{q.vendor?.city} · {q.vendor?.partnerLevel}</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
              ...
              <div className="border-t border-gray-100 p-4">
                <div className="grid grid-cols-3 gap-4 mb-4">
                  <div>
                    ...
                    {[['Contact',q.vendor?.contactPerson],['Phone',q.vendor?.phone],['City',q.vendor?.city],['Level',q.vendor?.partnerLevel],['Brands',q.vendor?.brands?.join(', ')]].map(([l,v])=>v&&(
                  ...
                  <div>
                    ...
                    {[['Total',`PKR ${fmt(Number(q.totalAmount))}`],['Delivery',q.deliveryDays],['Warranty',q.warranty],['Payment',q.paymentTerms],['Valid',q.validityDays?`${q.validityDays} days`:null]].map(([l,v])=>v&&(
                      <div key={l} className="flex gap-2 text-xs mb-1.5">
                        <span className="text-gray-400 w-16 flex-shrink-0">{l}</span>
                        <span className="text-gray-700 font-medium">{v}</span>
                      </div>
                      ...
                      <tbody>
                        {lines.map((item:any,idx:number)=>(
                          <tr key={idx} className="border-t border-gray-50">
                            <td className="px-3 py-2 text-gray-700">{item.desc||item.description||'—'}</td>
