...
export default function PostRFQForm({ postFee, bizCredits }: { postFee: number; bizCredits: number }) {
  ...
  const [form, setForm] = useState({
    title: '', brand: 'Dell', category: 'Laptops',
    quantity: '', budgetPkr: '', city: 'Karachi', specs: '',
  })
  ...
  async function submit() {
    ...
    if (bizCredits < postFee) {
      setError(`You need ${postFee} credits to post an RFQ but you only have ${bizCredits}. Please ask admin to add credits.`)
      return
    }
  ...
  if (done) return (
    <div className="bg-green-50 border border-green-200 rounded-xl p-6">
      ...
      <div className="text-sm text-green-700 mb-1">{postFee} credits have been deducted from your account.</div>
      <div className="text-sm text-green-600 mb-4">Admin will review and approve your RFQ shortly.</div>
      <div className="flex gap-3">
        <button onClick={() => { setDone(false); setForm({ title:'',brand:'Dell',category:'Laptops',quantity:'',budgetPkr:'',city:'Karachi',specs:'' }) }}
          className="text-sm border border-green-300 text-green-800 px-4 py-2 rounded-lg">Post another RFQ</button>
  ...
  return (
    <div className="space-y-4">
      <div className={`rounded-xl p-4 flex items-center justify-between ${bizCredits >= postFee ? 'bg-blue-50 border border-blue-100' : 'bg-red-50 border border-red-200'}`}>
        <div>
          <div className={`text-sm font-medium ${bizCredits >= postFee ? 'text-blue-700' : 'text-red-700'}`}>
            Posting fee: {postFee} credits per RFQ
          </div>
          <div className={`text-xs mt-0.5 ${bizCredits >= postFee ? 'text-blue-500' : 'text-red-500'}`}>
            Your balance: {bizCredits} credits — {bizCredits < postFee ? 'insufficient, contact admin to top up' : 'sufficient to post'}
          </div>
        </div>
        <div className={`text-2xl font-bold ${bizCredits >= postFee ? 'text-blue-600' : 'text-red-600'}`}>{bizCredits}</div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5 space-y-4">
        <div>
          ...
          <input value={form.title} onChange={e => set('title', e.target.value)}
            ...
            placeholder="e.g. 50x Dell Latitude 5540 Laptops for Office" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-gray-400 block mb-1">Brand required</label>
        ...
        <div>
          <label className="text-xs text-gray-400 block mb-1">Delivery city</label>
        ...
        <div>
          ...
          <textarea value={form.specs} onChange={e => set('specs', e.target.value)} rows={4}
            ...
            placeholder="Describe processor, RAM, storage, warranty, delivery timeline..." />
        </div>
        <div className="bg-amber-50 text-amber-700 text-xs p-3 rounded-lg">
          Do not include your company name or contact details in the description.
        </div>
        {error && <div className="text-xs text-red-600 bg-red-50 p-3 rounded-lg">{error}</div>}
        <div className="flex justify-end pt-1">
          <button onClick={submit} disabled={loading || bizCredits < postFee}
