...
function UserTable({ users, type }: { users: any[]; type: 'vendor' | 'business' }) {
  ...
  async function act(userId: string, action: 'approve' | 'reject') {
    ...
    setLoading(null)
    if (res.ok) setList(l => l.map(u => u.id === userId ? { ...u, status: action === 'approve' ? 'approved' : 'rejected' } : u))
...
git add .
git commit -m "feat: admin edit users, reset passwords, add/deduct credits"
git push
cat > "components/admin/AdminUserTable.tsx" << 'EOF'
'use client'
import { useState } from 'react'
import Link from 'next/link'
