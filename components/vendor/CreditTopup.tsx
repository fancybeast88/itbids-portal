...
type Txn = { id: string; credits: number; amountPkr: number | null; paymentMethod: string | null; status: string; createdAt: Date }
...
export default function CreditTopup({ packages, transactions, vendorEmail }: { packages: Pkg[]; transactions: Txn[]; vendorEmail: string }) {
  ...
  if (success) return (
    <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center">
      ...
      <div className="text-xs text-gray-400">Confirmation email will be sent to {vendorEmail}</div>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-4">
        <div className="text-xs font-medium text-gray-600 mb-3">Select package</div>
      ...
      {/* Payment method */}
      <div className="bg-white border border-gray-100 rounded-xl p-4">
        <div className="text-xs font-medium text-gray-600 mb-3">Payment method</div>
        <div className="space-y-2">
          {[
            ...
            { key: 'bank', label: 'Bank transfer', desc: 'HBL / MCB / UBL · Confirmed by admin', color: 'bg-blue-500' },
          ].map((m: any) => (
        ...
        {(method === 'jazzcash' || method === 'easypaisa') && (
          <div className="mt-3">
            <label className="text-[10px] text-gray-400 block mb-1">{method === 'jazzcash' ? 'JazzCash' : 'Easypaisa'} mobile number</label>
        ...
        {method === 'bank' && (
          <div className="mt-3 bg-gray-50 rounded-lg p-3 text-xs text-gray-600 space-y-1">
            ...
            <div>Account name: <strong>IT Bids Portal Pvt Ltd</strong></div>
            <div>Account no: <strong>0123-456789-01</strong></div>
            <div>Branch: <strong>Karachi Main</strong></div>
            <div className="text-amber-700 mt-1">Use your registered email as reference. Credits are added after admin confirms your transfer (1–2 business hours).</div>
          </div>
        )}
      </div>
      ...
      {transactions.length > 0 && (
        <div className="bg-white border border-gray-100 rounded-xl p-4 mt-2">
          <div className="text-xs font-medium text-gray-600 mb-3">Transaction history</div>
          <div className="space-y-2">
            {transactions.map((t: any) => (
              <div key={t.id} className="flex items-center justify-between text-xs py-1.5 border-b border-gray-50 last:border-0">
                <div>
                  ...
                  <span className="text-gray-400 ml-2">{t.paymentMethod || t.type?.replace('_',' ')} · {new Date(t.createdAt).toLocaleDateString('en-PK', { day: 'numeric', month: 'short' })}</span>
                </div>
                <span className={`text-[10px] px-2 py-0.5 rounded-full ${txnStatusColor[t.status]}`}>{t.status}</span>
              </div>
            ))}
          </div>
        </div>
